...
{
    public class TriggerRadiusIdAjaxDropdownController : Controller
    {
        private IRepo<TriggerRadius> repo;
        public TriggerRadiusIdAjaxDropdownController(IRepo<TriggerRadius> repo)
        {
            this.repo = repo;
        }
        ...
        public ActionResult GetItems(int? v)
        {
            ...
            {
                Text = string.Format("{0}", o.Distance),
                Value = o.Id.ToString(),
                Selected = o.Id == v
            }));
            return Json(list);
